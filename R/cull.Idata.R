...
#'currently culling on Nio values can be switched to Nio_weighted etc. etc.
#'1/2/3 standard deviations from mean ticxit
#'10%/20% MAD median(abs((ticxit)i-mean(ticxit)))
#' @param filter.Idata_results =results from filter.Idata function
#' @param it <= it value defined by user
#' @param ratio = isotope ratio calcualtion method preferred by user i.e. either "NL_ratio", "Nio_ratio", "Nio_w_ratio". Doesnt work for background subtracted ratios since negative values have to be replaced by NA first
